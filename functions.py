...
def bb_trading(stock, budget=15000, u=0.03, l=0.03, show_graph=True, show_return=True):
    ...
    money = budget
    stock['tp']=(stock.close_price+stock.intraday_high_price+stock.intraday_low_price)/3
    ...
    for i in range(len(stock)):
        ...
        # Buy
        if (today.tp < today.MA15) and (abs(1 - today.tp / today.Lower_band) < u):
            if money == 0:
                money = stock_amt * today.tp
                stock_amt = 0
                net.append([today.open_time, today.tp, 0, money])

        # Sell        
        if (today.tp > today.MA15) and (abs(1 - today.Upper_band / today.tp) < l):
            if money > 0:
                stock_amt = int(money / today.tp)
                money = 0
                net.append([today.open_time, today.tp, 1, stock_amt * today.tp])
    ...
    if show_graph == True:
        plotting.trading_history(stock.set_index('open_time'), net)
